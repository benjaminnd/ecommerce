...
function CartPage({
  ...
  useEffect(() => {
    ...
    const totalReducer = (accumulator, currentValue) =>
      accumulator + currentValue.price * currentValue.quant;
  ...
  useEffect(() => {
    ...
    const totalReducer = (accumulator, currentValue) =>
      accumulator + currentValue.price * currentValue.cartQuant;
    if (guestCart.length > 0) {
      setShowEmptyGuest(false);
      setGuestTotal(guestCart.reduce(totalReducer, 0));
    } else {
      setShowEmptyGuest(true);
    }
  }, [guestCart]);
  ...
  const guestTransactionSuccess = (data) => {
    ...
    let payment = {
      cartDetail: GuestCart,
      paymentData: data,
    };
    console.log("payment details", payment);

    axios
      .post(`${serverURL}/api/user/successPayGuest`, payment)
      .then((response) => {
        if (response.data.success) {
          toast.success("Payment successfully received");
  ...
  return (
    <>
      <div className="w-5/6 m-12 m-auto">
        <div className="">
          <h1 className="font-bold text-xl pb-2 my-6">Cart Items</h1>
        </div>
        <div>
          {!isAuth && showEmptyGuest && (
            <p className="italic">No items to show</p>
          )}
          {isAuth && showEmptyUser && (
            <p className="italic">No items to show</p>
          )}
          {isAuth && userCart.length > 0 && (
            <CartTable
              cart={userCartDetail}
              removeItem={removeItemUser}
          ...
          {!isAuth && guestCart.length > 0 && (
            <CartTable
              cart={guestCart}
              removeItem={removeItemGuest}
              isAuth={isAuth}
              handleQuantity={guestHandleQuantity}
            />
          )}
        </div>
