...
const NavbarList = ({ history, logout, cart, cartSum, userCart, isAuth }) => {
  ...
  useEffect(() => {
    ...
    const guestReducer = (accumulator, currentValue) =>
      accumulator + currentValue.cartQuant;
    if (isAuth) {
  ...
  return (
    <>
        ...
        <ul id="menuItems">
          ...
          {isAuth ? (
            <NavItem
              link="/upload"
              name="Upload"
              listStyle={isActive(history, "/upload")}
            />
          ) : (
            ""
          )}
      ...
      <Badge count={isAuth ? userCartCount : CartCount}>
        <a href="/user/cart">
          <img
            src={`${serverURL}/uploads/cart.png`}
            alt="cart"
            width="40px"
            height="50px"
          />
